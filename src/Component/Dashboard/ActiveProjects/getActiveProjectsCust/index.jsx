...
class index extends Component {
  ...
  componentWillMount() {
    if (cookie.load("Token")) {
      ...
      this.setState({ userId: token1.ReffID });
    }
  }
  componentDidMount() {
    console.log("user_id", this.state.userId);
    fetch(
      `${baseUrl}api/services/app/Project/GetActiveProjectByCustId?CustId=` +
        this.state.userId,
      {
        headers: {
      ...
      .then(json => {
        if (json.success) {
          ...
          let Workspace = this.props.t("workspace");
    ...
    const show = 
      items.length === 0 ? (
        isDataAvailable === false ? (
          ...
          <h5 className="text-center pt-5">{this.props.t("zero_projects")}</h5>
        )
      ) : (
        <div>
          <section className="">
            <div
              className="container body-container mt-5 d-flex justify-content-center"
              style={{ marginTop: "10%" }}
            >
              <div
        ...
        <div class="col-md-12">
          ...
          >
            <Link to="/myproject">
              ...
              >
                {this.props.t("add_new")} <i className="fal fa fa-long-arrow-right"></i>
              </button>
            </Link>
          </div>
        </div>
